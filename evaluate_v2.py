...
model   = load_model("models/best_model_v2.keras")

y_pred_probs = model.predict(X_test, verbose=1)
y_pred = np.argmax(y_pred_probs, axis=1)

acc = np.mean(y_pred == y_test)
print(f"\nUkupna tacnost V2 modela na test setu: {acc*100:.2f}%\n")

print("=" * 60)
print("TACNOST PO KLASAMA (V2):")
print("=" * 60)
...
if bad_classes:
    ...
    print("\nNema klasa ispod praga 0.85! Svi znaci se dobro prepoznaju.")
...
plt.title('F1 score po klasi - V2 model sa curl feature-ima (crveno = problem)')
plt.xlabel('Slovo')
plt.ylabel('F1 score')
plt.xticks(rotation=45)
plt.legend()
plt.tight_layout()
plt.savefig('evaluacija_po_klasama_v2.png', dpi=150)
plt.show()

print("\nGrafik sacuvan kao 'evaluacija_po_klasama_v2.png'")