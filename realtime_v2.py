...
# ============================================================
# CURL FEATURE-I (savijenost prstiju) - moraju se racunati isto
# kao u prepare_dataset_v2.py, jer je model treniran sa njima
# ============================================================
WRIST = 0
...
with mp_holistic.Holistic(min_detection_confidence=0.3,
    ...
    while cap.isOpened():
        ...
        if state == IDLE:
            ...
            X_with_curl = add_curl_features_to_sequence(X_raw)        # (40, 268)
            X_2d = X_with_curl.reshape(1, NUM_FRAMES * X_with_curl.shape[1])
            X_scaled = scaler.transform(X_2d)
            X_3d = X_scaled.reshape(1, NUM_FRAMES, X_with_curl.shape[1])
